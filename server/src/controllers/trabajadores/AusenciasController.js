const { Ausencia, AusenciaRepository } = require("../../models/trabajadores/ausencia");
...
const insertAusencia = async (req, res) => {
  try {
    ...
    const newAusencia = new Ausencia(idTrabajador, fechaAusencia, justificada);

    await ausenciaRepo.insertAusencia(newAusencia);
...
const updateAusencia = async (req, res) => {
  try {
    const idAusencia = req.params.idAusencia;
...
const deleteAusencia = async (req, res) => {
  try {
    const idAusencia = req.params.idAusencia;
    const actualizacionExitosa = await ausenciaRepo.deleteAusencia(idAusencia);
